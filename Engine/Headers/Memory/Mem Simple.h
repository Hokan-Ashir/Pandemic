...
T1(const_mem_addr TYPE) struct Mems // Simple Continuous Memory Based Container
{
 ...
 C TYPE& last      (     )C; // get    last  element
   ...
   T1(VALUE) Bool  exclude(C VALUE &value)  {Int i=find(value); if(i!=-1){remove(i);   return true ;}             return false;} // exclude  'value' if present  in container                , returns true if value was    present and has been removed, this method changes the memory address of all elements
   T1(VALUE) Bool  toggle (C VALUE &value)  {Int i=find(value); if(i!=-1){remove(i);   return false;} add(value); return true ;} // toggle   'value'    presence in container                , returns true if value is now present in container        , this method changes the memory address of all elements
   ...
   T1(VALUE)   Mems& binaryAdd    (C VALUE &value,             Int compare(C TYPE &a, C VALUE &b))  {Int i;        binarySearch(value, i, compare); NewAt (i)=value;                          return     T;} // add      'value' (using binary search)                                                                                                    , this method changes the memory address of all elements
   ...
   T1(VALUE)   Bool  binaryExclude(C VALUE &value,             Int compare(C TYPE &a, C VALUE &b))  {Int i; if(    binarySearch(value, i, compare)){remove(i)      ; return true;}            return false;} // exclude  'value' (using binary search) if present  in container                , returns true if value was    present and has been removed, this method changes the memory address of all elements
   T1(VALUE)   Bool  binaryToggle (C VALUE &value,             Int compare(C TYPE &a, C VALUE &b))  {Int i; if(   !binarySearch(value, i, compare)){NewAt (i)=value; return true;} remove(i); return false;} // toggle   'value' (using binary search)    presence in container                , returns true if value is now present in container        , this method changes the memory address of all elements
   ...
   template<Int size> Mems& operator=(C Memp<TYPE, size> &src); // copy elements using assignment operator
