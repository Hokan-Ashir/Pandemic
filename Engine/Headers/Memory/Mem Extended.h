...
T1(TYPE) struct Memx : _Memx // Block Based Extended Container
{
 ...
 C TYPE& last      (     )C; // get last  valid    element
   ...
   T1(VALUE)   Memx& binaryAdd    (C VALUE &value,             Int compare(C TYPE &a, C VALUE &b))  {Int i;        binarySearch(value, i, compare); NewAt      (i)=value;                                     return     T;} // add      'value' (using binary search)                                                                                                    , this method does not change the memory address of any of the elements
   T1(VALUE)   Bool  binaryInclude(C VALUE &value,             Int compare(C TYPE &a, C VALUE &b))  {Int i; if(   !binarySearch(value, i, compare)){NewAt      (i)=value; return true;}                       return false;} // include  'value' (using binary search) if it's not already present in container, returns true if value wasn't present and has been added  , this method does not change the memory address of any of the elements
   T1(VALUE)   Bool  binaryExclude(C VALUE &value,             Int compare(C TYPE &a, C VALUE &b))  {Int i; if(    binarySearch(value, i, compare)){removeValid(i, true); return true;}                       return false;} // exclude  'value' (using binary search) if present  in container                , returns true if value was    present and has been removed, this method does not change the memory address of any of the elements
   T1(VALUE)   Bool  binaryToggle (C VALUE &value,             Int compare(C TYPE &a, C VALUE &b))  {Int i; if(   !binarySearch(value, i, compare)){NewAt      (i)=value; return true;} removeValid(i, true); return false;} // toggle   'value' (using binary search)    presence in container                , returns true if value is now present in container        , this method does not change the memory address of any of the elements
   ...
   template<Int size> Memx& operator=(C Memp<TYPE, size> &src); // copy elements using assignment operator
...
protected:
   explicit Memx(Abstract, Int block_elms=32); // 'block_elms'=number of elements per block
};
/******************************************************************************/
T1(TYPE) struct MemxAbstract : Memx<TYPE> // Block Based Extended Container which allows storage of abstract classes, 'replaceClass' should be called before creating new elements in it
{
   explicit MemxAbstract(Int block_elms=32) : Memx<TYPE>(_Memx::Abstract(), block_elms) {} // 'block_elms'=number of elements per block
};
/******************************************************************************/
