...
/******************************************************************************/
enum PROP_MOUSE_EDIT_MODE // Property Mouse Edit Mode
{
...
const_mem_addr struct Property // Class Member Gui Control !! must be stored in constant memory address !!
{
   ...
   Property& set(  Bool  value, Bool quiet=false);   Bool asBool()C;   Bool asBool(CPtr object)C {return md.asBool(object                );} // set  /  get from gui  /  get from data, 'quiet'=if set value in quiet mode where no additional functions will be called
   Property& set(  Int   value, Bool quiet=false);   Int  asInt ()C;   Int  asInt (CPtr object)C {return md.asInt (object                );} // set  /  get from gui  /  get from data, 'quiet'=if set value in quiet mode where no additional functions will be called
   Property& set(  Flt   value, Bool quiet=false);   Flt  asFlt ()C;   Flt  asFlt (CPtr object)C {return md.asFlt (object                );} // set  /  get from gui  /  get from data, 'quiet'=if set value in quiet mode where no additional functions will be called
   Property& set(C Vec2 &value, Bool quiet=false);   Vec2 asVec2()C;   Vec2 asVec2(CPtr object)C {return md.asVec2(object                );} // set  /  get from gui  /  get from data, 'quiet'=if set value in quiet mode where no additional functions will be called
   Property& set(C Str  &value, Bool quiet=false);   Str  asText()C;   Str  asText(CPtr object)C {return md.asText(object, real_precision);} // set  /  get from gui  /  get from data, 'quiet'=if set value in quiet mode where no additional functions will be called
                                                                       Int  asEnum(CPtr object)C {return md.asEnum(object                );} // set  /  get from gui  /  get from data, 'quiet'=if set value in quiet mode where no additional functions will be called
...
void SaveProperties(C Memx<Property> &properties, Memp<TextNode> nodes, Bool handle_same_names=false); // save properties to   text nodes, 'handle_same_names'=if handle case when some properties have same names
void LoadProperties(  Memx<Property> &properties, Memp<TextNode> nodes, Bool handle_same_names=false); // load properties from text nodes, 'handle_same_names'=if handle case when some properties have same names
...
void LoadProperties(  Memx<Property> &properties, XmlNode &node, Char space_replacement='\0'); // load properties from xml node, since xml does not support parameters with spaces in their names, all spaces need to be replaced, you can use 'space_replacement' to specify custom character as the replacement
/******************************************************************************/
