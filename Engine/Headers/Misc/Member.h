...
struct MemberDesc // Member Description - Description of a Member in a C++ class
{
   ...
   Bool      compare_number; // if compare strings using 'CompareNumber' instead of 'Compare' function, default=true
   ...
   T1(TYPE) TYPE& as(Ptr object                   ) {return *(TYPE*)((Byte*)object+offset);} // get reference to member, 'object'=pointer to C++ object of class that this member belongs to !! specified TYPE should be the exact same type of the class member !!
   ...
   template<Int elms>   MemberDesc& set(Char8   (&member)[elms]) {return setDefault(DATA_CHAR8    , member);}
                        ...
                        MemberDesc& set(VecB4    &member       ) {return setDefault(DATA_VECI4    , member);}
...
private:
   ...
   T1(TYPE) MemberDesc& setDefault(DATA_TYPE type, TYPE &member) {return set(type, UIntPtr(&member), SIZE(member));}
};
/******************************************************************************/
