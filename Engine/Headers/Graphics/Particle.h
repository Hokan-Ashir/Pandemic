...
/******************************************************************************/
enum PARTICLE_SRC // Particles Source Type
{
   PARTICLE_NONE                 , // none
...
struct Particles // Set of Particles
{
   ...
   Bool  reborn              , // if reborn particles after their death                                                , false/true, default=true
         smooth_fade         , // false for (quick fade-in and slow fade-out) or true for (smooth fade-in and fade-out), false/true, default=false
   ...
   Bool        is         (             )C {return _src_type!=PARTICLE_NONE                            ;} // if  Particle Set has been created
   ...
   // operations
   void       reset (Int i          ); // reset i-th particle
   Particles& reset (               ); // reset  all particles
   ...
   // io
   void save(  File &f   , Bool include_particles, CChar *path=NULL)C; // save, does not include  saving dynamic sources               , 'include_particles'=if include each single particle data in saving (if not, then they will be set randomly when loading), 'path'=path at which resource is located (this is needed so that the sub-resources can be accessed with relative path)
