...
struct Environment
{
   ...
   struct Bloom
   {
      Bool on        , // if enabled      , true/false, default=true
           ...
           overbright; // bloom overbright, true/false, default=false
      Byte blurs     ; // number of blurs ,    1..4   , default=2
      Flt  original  , // original color  ,    0..Inf , default=1.0
           scale     , // bloom scale     ,    0..Inf , default=0.6
           cut       , // bloom cutoff    ,    0..Inf , default=0.3
           contrast  ; // bloom contrast  ,    0..Inf , default=1.0
   ...
   struct Sky
   {
      ...
      Flt      atmospheric_density_exponent , // atmospheric density exponent,            0..1                   , default=1, (1 is the fastest)
               atmospheric_horizon_exponent ; // atmospheric horizon exponent,            0..Inf                 , default=4, (this affects at what height the horizon color will be selected instead of the sky color)
      Vec4     atmospheric_horizon_color    , // atmospheric horizon color   ,    (0,0,0,0)..(1,1,1,1)           , default=(0.40, 0.48, 0.64, 1.0) here alpha specifies opacity to combine with star map when used
               atmospheric_sky_color        ; // atmospheric sky     color   ,    (0,0,0,0)..(1,1,1,1)           , default=(0.27, 0.34, 0.49, 0.9) here alpha specifies opacity to combine with star map when used
   ...
   struct Sun
   {
      ...
      Vec      pos            , // position on sky sphere      , its length must be equal 1, default=!Vec(-1, 1, -1)
               light_color    , // light color                 ,     (0,0,0)..(1,1,1)      , default=(0.7, 0.7, 0.7), value of (0, 0, 0) disables light casting
                rays_color    ; // rays  color                 ,     (0,0,0)..(1,1,1)      , default=(0.1, 0.1, 0.1)
