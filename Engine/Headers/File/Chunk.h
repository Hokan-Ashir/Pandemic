...
struct ChunkWriter // Writes a custom amount of chunks into a single file
{
   ...
   void  beginChunkList(const_mem_addr File &f   ); // begin    writing a  new      chunk list to 'f' file, this method writes "chunk list marker" every time               , 'f' shouldn't be modified (or closed) during chunk data saving, because 'ChunkWriter' relies on 'f', automatically calls 'endChunkList' if needed
   File* beginChunk    (C Str &name, UInt version); // begin    writing a 'name' chunk in current file with 'version', this method returns a pointer to a file where you should write your custom data, NULL on fail, automatically calls 'endChunk' if needed
   void    endChunk    (                         ); // end      writing a        chunk in current file
   void    endChunkList(                         ); // end      writing chunk list, this method writes an ending marker at the end of all chunks, automatically calls 'endChunk' if needed

   ChunkWriter(                      );
   ChunkWriter(const_mem_addr File &f); // automatically calls 'beginChunkList'
...
struct ChunkReader // Processes chunks from a single file
{
 ...
 C Str & name      ()C {return _name;} // get name    of currently processed chunk
   UInt  ver       ()C {return _ver ;} // get version of currently processed chunk

   ChunkReader(                      );
   ChunkReader(const_mem_addr File &f); // automatically calls "read(f)"

private:
   UInt  _ver;
   ULong _next_chunk_pos, _temp_size, _temp_offset;
   Str   _name;
   File *_f;
