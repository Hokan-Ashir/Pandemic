...
/******************************************************************************
   ...
   Use 'Drive' and 'GetDrives' to access your computer drives information.
...
// DRIVE
/******************************************************************************/
enum DRIVE_TYPE // Drive Type
{
   DRIVE_UNDEFINED, // undefined
   DRIVE_HDD      , // hard disk drive
   DRIVE_OPTICAL  , // optical   (CD/DVD/..)
...
// FILE INFO
/******************************************************************************/
enum FSTD_TYPE // Standard File Type
{
   FSTD_NONE , // none
   FSTD_DRIVE, // drive
   FSTD_DIR  , // folder
...
//{
   Str name; // encountered file base name (does not include path)

   // operations
   void find      (C Str &path, C Str &ext=S); // start searching in 'path', accept only 'ext' extensions (set NULL for all)
 ...
 C Str&       path      ()C {return _path      ;} // get searching path
  ...
  ~FileFind(                         );
   FileFind(                         );
   FileFind(C Str &path, C Str &ext=S);
...
struct BackgroundFileFind
{
  ...
  ~BackgroundFileFind() {del();}
   BackgroundFileFind() {}
   BackgroundFileFind(C Str &path, Bool (*filter)(C Str &name)=NULL) {find(path, filter);}
...
void GetDrives(Memp<Drive> drives); // get Drives in the system

Str  CurDir(          ); // get current working directory, by default it's always set to path of the application exe file (an exception is iOS platform, there the path is set inside the executable, since the executable is actually a folder)
...
Bool FEqual      (            C Str     &a  , C Str &b   ,                                                      Secure *  a_secure=NULL, Secure *   b_secure=NULL                                   ); // if files have the same data
Bool FCopy       (            C Str     &src, C Str &dest, FILE_OVERWRITE_MODE overwrite=FILE_OVERWRITE_ALWAYS, Secure *src_secure=NULL, Secure *dest_secure=NULL, CChar *safe_overwrite_suffix=NULL); // copy    'src'     file  to 'dest' file  , false on fail, 'safe_overwrite_suffix'=if specified then 'SafeOverwrite' function will be used
Bool FCopy       (  Pak &pak, C PakFile &src, C Str &dest, FILE_OVERWRITE_MODE overwrite=FILE_OVERWRITE_ALWAYS,                          Secure *dest_secure=NULL, CChar *safe_overwrite_suffix=NULL); // copy    'src' pak file  to 'dest' file  , false on fail, 'safe_overwrite_suffix'=if specified then 'SafeOverwrite' function will be used
Bool FCopy       (  Pak &pak, C Str     &src, C Str &dest, FILE_OVERWRITE_MODE overwrite=FILE_OVERWRITE_ALWAYS,                          Secure *dest_secure=NULL, CChar *safe_overwrite_suffix=NULL); // copy    'src' pak file  to 'dest' file  , false on fail, 'safe_overwrite_suffix'=if specified then 'SafeOverwrite' function will be used
Bool FCopy       (  Pak &src                , C Str &dest, FILE_OVERWRITE_MODE overwrite=FILE_OVERWRITE_ALWAYS,                          Secure *dest_secure=NULL                                   ); // extract 'src' pak files to 'dest' folder, false on fail
Bool FCopyDir    (C Str &src                , C Str &dest, FILE_OVERWRITE_MODE overwrite=FILE_OVERWRITE_ALWAYS, Secure *src_secure=NULL, Secure *dest_secure=NULL                                   ); // copy    'src' folder    to 'dest' folder, false on fail
Bool FReplaceDir (C Str &src                , C Str &dest, FILE_OVERWRITE_MODE overwrite=FILE_OVERWRITE_ALWAYS, Secure *src_secure=NULL, Secure *dest_secure=NULL                                   ); // copy    'src' folder    to 'dest' folder and remove all contents of 'dest' which are not present in 'src', false on fail
...
Long FSize       (C Str &name                      ); // get     file/folder size   in the system
...
Bool FDelPath    (C Str &name                      ); // delete  elements   , false on fail (this will delete all elements inside provided path)
...
Bool SafeOverwrite(File &src, C Str &dest, C DateTime *modify_time_utc=NULL, Secure *dest_secure=NULL, C Str &suffix="@new", ReadWriteSync *rws=NULL); // safely overwrite 'dest' file from 'src' data, this first writes the full data (starting from current 'src' position) to a temporary name made from dest+suffix, after the file has been written successfully, its modification time is set to 'modify_time_utc' if not NULL, after that the file name is changed to the final 'dest' replacing any existing file at that location, 'rws'=(optional) if specified then it will have its 'enterWrite' called just before replacing the file and 'leaveWrite' afterwards, false on fail
