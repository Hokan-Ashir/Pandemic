...
struct Motion // Animation Motion, helper class for playing a single Animation
{
   ...
   Animation* animation  ()C {return skel_anim ? skel_anim->animation() : NULL   ;} // get animation
   ...
   Bool eventOccured (CChar8 *name            )C; // if                         'name' event (in the animation) occured in current frame
   Bool eventOccured (Flt     event_time      )C; // if                                event time               occured in current frame
