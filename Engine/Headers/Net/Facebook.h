   ...
   In order to use 'Facebook' class, you need to:
      ...
      1. Create a "Facebook App" on https://developers.facebook.com/apps/ with exact same name as your Esenthel Application
         ...
         8. For "Android Platform" set "Key Hashes" from:
            1. In Esenthel Editor, open Code Editor options "Build\Show Options"
...
struct Facebook
{
   ...
   struct Score
   {
      ...
      Score& set(ULong user_id, Int score) {T.user_id=user_id; T.score=score; return T;}
   };
   ...
   Facebook& getScores (         ); // request  scores for current app        to be downloaded, 'logIn' will be automatically called if needed, for this to work the category of your Facebook App needs to be set to "Games", this will download yours and your friends scores in this Facebook App
   Facebook& setScore  (Int score); // set your score  for current app                        , 'logIn' will be automatically called if needed, for this to work the category of your Facebook App needs to be set to "Games"

 C      User  & me       ()C {return _me        ;} // get my         profile information, 'getMe'      should be called first
 C Mems<User >& friends  ()C {return _friends   ;} // get my friends profile information, 'getFriends' should be called first
 C Mems<Score>& scores   ()C {return _scores    ;} // get scores for current app        , 'getScores'  should be called first
 C DateTime   & scoresUTC()C {return _scores_utc;} // get date and time of latest scores in UTC time zone
