...
//{
   ...
   ObjParamsPtr     obj           ; // object parameter pointer
   ...
   // manage
   virtual void create(ObjParams &obj); // create from object parameters, Chr class accepts only 'PhysBody' made from 1 Capsule (ignoring its "up" vector), if no PhysBody is given then the character controller will be created basing on the object scale
   ...
   virtual void animateStepOccured() {} // this method is called when "step" event has occured in the most significant movement animation, you can override this method and for example play "step" sound
...
public: // following members/methods are public, however don't modify/call them manually unless you know what you're doing
   ...
   struct SkelAnimCache // Skeleton Animation Cache, these are the pointers to character default animations, you can replace them with custom ones, for example 'walk=skel.getSkelAnim("custom walk.anim")'
   {
      ...
      Int head, neck, body, body_u, arm_ru, arm_lu, arm_rd, arm_ld, leg_lu, leg_ru, leg_ld, leg_rd, hand_l, hand_r, foot_l0, foot_r0; // indexes of skeleton bones found in 'skel', -1=none

      void set(AnimatedSkeleton &anim_skel, C ObjParamsPtr &obj); // set default values using 'skel' skeleton
