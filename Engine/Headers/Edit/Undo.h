...
struct _Undo // Undo (do not use this class, use 'Undo' below)
{
  ...
  _Undo& forceCreateNextUndo(); // always allow the next 'set' call to create a new Undo Change
   ...
   Long memUsage()C;  _Undo& memUsage(Long limit); // get current memory  usage / set max allowed memory usage for all Undo Changes (-1=unlimited, default=-1)
   Int  changes ()C;  _Undo& changes (Int  limit); // get current changes count / set max allowed number of        all Undo Changes (-1=unlimited, default=-1)
...
private:
  ...
  _Undo(Bool full, Ptr user, Flt time);
   T1(TYPE) friend struct Undo;
};
/******************************************************************************/
T1(TYPE) struct Undo : _Undo // 'TYPE' must be based on '_Undo.Change'
{
   Undo(Bool full, Ptr user=NULL, Flt time=1.0f); // 'full'=if use "Full" or "Partial" mode, 'user'=custom user data that will be passed to 'Change' methods, 'time'=max time between multiple changes to consider them as one
