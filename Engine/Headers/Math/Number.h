...
struct Number // Number with big precision (NUMBER_DIGS*16 bit precision)
{
   ...
   // add / subtract / multiply / divide / mod
          Number& operator+=(C Number &N);   Number& operator+=(Int);   Number& operator+=(Dbl);   void operator++() {T+=1;}   void operator++(Int) {T+=1;} // add
          Number& operator-=(C Number &N);   Number& operator-=(Int);   Number& operator-=(Dbl);   void operator--() {T-=1;}   void operator--(Int) {T-=1;} // subtract
