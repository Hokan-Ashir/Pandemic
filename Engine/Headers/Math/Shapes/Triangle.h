...
Vec  GetNormalU(C Vec  &p0, C Vec  &p1, C Vec  &p2); // calculate un-normalized normal vector from 3 triangle points (length of the vector is proportional to the area of the triangle)
/******************************************************************************/
struct Tri2 // Triangle 2D
{
   ...
   void draw(C Color &color=WHITE, Bool fill=true)C;
          ...
          Tri2& operator*=(  Flt   r);
          Tri2& operator/=(  Flt   r);
   friend Tri2  operator+ (C Tri2 &tri, C Vec2 &v);
   friend Tri2  operator- (C Tri2 &tri, C Vec2 &v);
   friend Tri2  operator* (C Tri2 &tri,   Flt   r);
   friend Tri2  operator/ (C Tri2 &tri,   Flt   r);
...
struct TriD2 // Triangle 2D (double precision)
{
   ...
   void draw(C Color &color=WHITE, Bool fill=true)C;
          ...
          TriD2& operator/=(  Dbl    r);
   friend TriD2  operator+ (C TriD2 &tri, C VecD2 &v);
   friend TriD2  operator- (C TriD2 &tri, C VecD2 &v);
   friend TriD2  operator* (C TriD2 &tri,   Dbl    r);
   friend TriD2  operator/ (C TriD2 &tri,   Dbl    r);
...
struct Tri // Triangle 3D
{
   ...
   Tri& setNormal() {n=GetNormal(p[0], p[1], p[2]); return T;} // recalculate normal
   ...
   void draw(C Color &color=WHITE, Bool fill=false)C; // this relies on active object matrix which can be set using 'SetMatrix' function
          ...
          Tri& operator*=(  Flt  r);
          Tri& operator/=(  Flt  r);
   friend Tri  operator+ (C Tri &tri, C Vec &v);
   friend Tri  operator- (C Tri &tri, C Vec &v);
   friend Tri  operator* (C Tri &tri,   Flt  r);
   friend Tri  operator/ (C Tri &tri,   Flt  r);
...
struct TriD // Triangle 3D (double precision)
{
   ...
   void draw(C Color &color=WHITE, Bool fill=false)C; // this relies on active object matrix which can be set using 'SetMatrix' function
          ...
          TriD& operator*=(  Dbl   r);
          TriD& operator/=(  Dbl   r);
   friend TriD  operator+ (C TriD &tri, C VecD &v);
   friend TriD  operator- (C TriD &tri, C VecD &v);
   friend TriD  operator* (C TriD &tri,   Dbl   r);
   friend TriD  operator/ (C TriD &tri,   Dbl   r);
