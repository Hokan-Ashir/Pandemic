...
struct Capsule // Capsule Shape
{
   ...
   // get
   Flt area  ()C {return PID2*r*   h            ;} // get surface area
   Flt volume()C {return PID *r*r*(h-r*(2.0f/3));} // get volume
   ...
   // transform
          ...
          Capsule& operator-=(C Vec     &v) {pos-=v; return T;}
          Capsule& operator+=(  Flt      f) {r  +=f; return T;}
          Capsule& operator-=(  Flt      f) {r  -=f; return T;}
          ...
          Capsule& operator/=(C Matrix  &m);
   friend Capsule  operator+ (C Capsule &capsule, C Vec     &v);
   friend Capsule  operator- (C Capsule &capsule, C Vec     &v);
   friend Capsule  operator* (C Capsule &capsule,   Flt      f);
   friend Capsule  operator/ (C Capsule &capsule,   Flt      f);
   friend Capsule  operator* (C Capsule &capsule, C Vec     &v);
   friend Capsule  operator/ (C Capsule &capsule, C Vec     &v);
   friend Capsule  operator* (C Capsule &capsule, C Matrix3 &m);
   friend Capsule  operator/ (C Capsule &capsule, C Matrix3 &m);
   friend Capsule  operator* (C Capsule &capsule, C Matrix  &m);
   friend Capsule  operator/ (C Capsule &capsule, C Matrix  &m);
...
Bool SweepCapsulePlane(C Capsule &capsule, C Vec &move, C Plane &plane, Flt *hit_frac=NULL, Vec *hit_pos   =NULL); // if moving capsule cuts through a static plane
