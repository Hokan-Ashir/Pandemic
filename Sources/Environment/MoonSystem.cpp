...
namespace pan {	
	...
	void MoonSystem::updateCelestialSphereParameters(const WorldChangingEvent* event) {
		...
		updateMoonPosition();
		createMaskedMoonImage(CurrentDateTime::getInstance()->getDayInMoonMonth());
		rotateMaskedMoonImage(event->getWorldLatitude());
	}
	...
	void MoonSystem::rotateMaskedMoonImage(const Flt worldLatitude) {
		...
		for (auto x = 0; x < width; ++x) {
			for (auto y = 0; y < height; ++y) {
				auto resultPixel = tmpImage.color(x, y);
				auto rotated = rotateVectorAroundOrigin(x, y, centerX, centerY, angle);
				maskedMoonImage.color(rotated.x, rotated.y, resultPixel);
			}
		}
	...
	void MoonSystem::createMaskedMoonImage(UShort dayInMoonMonth) {
		...
		for (auto x = 0; x < width; ++x) {
			for (auto y = 0; y < height; ++y) {
				...
				maskedMoonImage.colorF(x, y, resultPixel);
			}
		}
